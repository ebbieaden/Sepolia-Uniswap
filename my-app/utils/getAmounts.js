...
export const getEtherBalance = async (provider, address, contract = false) => {
    try {
        ...
        if (contract) {
            const balance = await provider.getEtherBalance(EXCHANGE_CONTRACT_ADDRESS);
            return balance;
        } else {
            const balance = await provider.getEtherBalance(address);
            return balance
        }
    } catch (err) {
        console.error(err);
...
export const getCDTokensBalance = async (provider, address) => {
    try {
        ...
        const balanceOfCryptoDevTokens = await exchangeContract.balanceOf(address);
